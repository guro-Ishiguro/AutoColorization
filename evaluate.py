...
def parse_args():
    p = argparse.ArgumentParser(description="Update leaderboard from submission summary")
    p.add_argument('-s','--sub-file', required=True,
                   help='Path to submission.csv (must contain date and score)')
    p.add_argument('-o','--out-file', default='leaderboard.csv',
...
def main():
    ...
    if 'score' not in df_sub.columns or 'date' not in df_sub.columns:
        print("Error: submission.csv must contain 'date' and 'score' columns", file=sys.stderr)
        sys.exit(1)
